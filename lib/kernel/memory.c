...
void pfree(uint32_t pg_phy_addr)
{
     ...
     bitmap_set(&mem_pool->pool_bitmap,bit_idx,0);//将位图中改位清零
}
//去掉页表中虚拟地址vaddr的映射，只去掉vaddr对应的pte
static void page_table_pte_remove(uint32_t vaddr)
{
     uint32_t*pte=pte_ptr(vaddr);
     *pte&=~PG_P_1; //将页表的项pte的p位制0
     asm volatile("invlpg %0"::"m"(vaddr):"memory");//更新tlb
}
//在虚拟地址池中释放掉以_vaddr起始的连续的pg_cnt个虚拟地址
...
void mfree_page(enum pool_flags pf,void*_vaddr,uint32_t pg_cnt)
{
     ...
     //确保释放的物理内存在低端1MB+1KB大小的页目录+1KB的页表地址范围之外
     ASSERT((pg_phy_addr%PG_SIZE)==0&&pg_phy_addr>=0x102000);
     //判断pg_phy_addr是属于用户物理内存池还是内核物理内存池
...
void sys_free(void*ptr)
{
     ...
     }else
     {
        //先将内存块收到free_list；
        list_append(&a->desc->free_list,&b->free_elem);
        //再判断此arena中的内存块是否都空闲,如果是就释放掉arena
